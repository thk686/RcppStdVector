...
There are several reasons one might wish to use ```std::vector``` over the native Rcpp vector types. First, as part of the Standard Library, relying on ```std::vector``` minimizes code rewrites when moving C++ code into Rcpp. Second, ```std::vector``` is simple and well-documented. Compare [this](https://en.cppreference.com/w/cpp/container/vector) to [this](http://dirk.eddelbuettel.com/code/rcpp/html/classRcpp_1_1Vector.html). Third, ```std::vector``` supports amortized-constant appending elements. This is a very common idiom in C++ coding and its absense in Rcpp is a source of frustration. Consider the following functions:
```
